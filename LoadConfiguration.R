...
for (i in seq_along(cdmSources)) {
  databases[[length(databases) + 1]] <- list(
    ...
    connectionDetails = DatabaseConnector::createConnectionDetails(
      dbms = "redshift",
      connectionString = keyring::key_get(paste0(config$keyringConnectionStringPrefix, cdmSources[i]), keyring = config$keyringName),
      # changed databaseId to cdmSources[i] because this was causing keyring to reuse optumDod on every sequence.
